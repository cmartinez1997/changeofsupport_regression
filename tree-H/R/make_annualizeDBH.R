...
backcalculate_DBH <- function(TRE_CN,DIA_t,MEASYEAR,Year,RW){
  ...
  if(length(N) > 0){
    ...
    tree_df$DIA_bc[N] <- tree_df$DIA_t[N] #dbh when year of ring width and measure year are equal
    while (Curr_row > 0 & !is.na(tree_df$DIA_bc[Curr_row + 1])) { #loop will stop when it gets to the end of data for that tree
      DIA_1 <- tree_df$DIA_bc[Curr_row+1] #or DIA_t[N] for the first round
      RW1 <- tree_df$RW[Curr_row+1] 
      #convert ring width from mm to inches
      RW1 = RW1 * 0.0393701
      tree_df$DIA_bc[Curr_row] <- DIA_1 - (2*RW1) ## okay this is just the absolute ring width way right now
